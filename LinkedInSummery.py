...
import streamlit as st
import openai
def generate_summary(profile_url):
    # You can customize the prompt based on your needs
    OPENAI_API_KEY='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'
    prompt = f"Generate a summary report for the LinkedIn profile at {profile_url}."
    ...
    response = openai.Completion.create(
        ...
        prompt=prompt,
        max_tokens=300  # You can adjust this based on the desired length of the summary
    )
    
    # Extract and return the generated summary from the OpenAI API response
...
if profile_url:
    st.write(f"Generating summary for the LinkedIn profile at {profile_url}...")

    # Call the function to generate the summary
    summary = generate_summary(profile_url)
